...
from fdic_tools import calculate_benchmark, calculate_growth_rates, get_filename, get_mapping, series_full_form, \
...
class Command(BaseCommand):
    help = 'import the latest quarter of NCUA data (ncua_import 2019-03). Optionally add a # of records: ncua_import ' \
           '2019-03 2 (for 2 records imported) '
    ...
    def handle(self, *args, **options):
        ...
        fn = get_filename(table)
        map_dict = get_mapping(table)

        series = list(map_dict.values())[1:]
        keys = list(map_dict.keys())[1:]
        ap = AnalyticsPlatform.objects.get(code=file)
        import_data(options['date_str'][0])
        # series_to_calculate = series_full_form(period)
        quarters_all = [quarter]
        if quarter == 'all':
            quarters_all = default_quarters_all
        fp = 'app/data/%s/' % file + options['date_str'][0]
        rows = load_csv_data(fn, fp)
        h = rows[0]
        h = "".join(h).split("\t")
        hl = [x.lower() for x in h]

        field_list = ['entity_id', 'entity_type', 'series', 'series_id', 'custom', 'year', 'analyticsplatform_id',
                      'base_data_interval']
        ...
        for i, m in enumerate(map_dict):
            ...
            for j, row in enumerate(rows[:num_records]):
                row = "".join(row).split("\t")
                if j != 0:
                    outrows.append([row[0], 'bank', map_dict[m], m, row[hl.index(m)], yr, ap.id, 'quarterly'])
        ...
        #                 sql = ('%s%s%s') % ("LOAD DATA INFILE '", s + '.csv',
        #                                     """'INTO TABLE ga.temp FIELDS TERMINATED by ',' OPTIONALLY ENCLOSED BY '"' LINES TERMINATED BY '\n' IGNORE 1 ROWS (entity_id,entity_type,series,series_id,custom,year,analyticsplatform_id,base_data_interval);""")
        #                 cursor.execute(sql)
        ...
        #                 sql = "UPDATE ga.temp SET ga.temp." + qtr + " = ga.temp.custom+0.0, ga.temp.custom=NULL, ga.temp.analyticsplatform_id=" + str(
        #                     ap.id) + ", ga.temp.base_data_interval='quarterly';"
        #                 cursor.execute(sql)
        ...
        #             else:
        #                 bulk = []
        #                 records = DataSummary.objects.filter(analyticsplatform=ap, year=int(yr), series=s,
        #                                                      entity_type='credit_union').values()
        #                 print(records.count())
        #                 for r in records:
        ...
        #         kwargs['series'] = s
        #         ds = DataSummary.objects.filter(analyticsplatform=ap, year=int(yr), entity_type='benchmark',
        #                                         series='Average ' + s).first()
                ...
                for state in states:
                    ...
                    if state:
                        ...
                        if qtr == 'q1':
                            ...
                            ds = DataSummary.objects.get(analyticsplatform=ap, year=int(yr), entity_type='bank',
                                                         series=state + ' Average ' + s)
