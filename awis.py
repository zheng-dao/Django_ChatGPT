...
def get_domain(call_type='UrlInfo', id=None, entity_type='credit_union'):
    pass

def get_all(call_type='UrlInfo', id=None, entity_type='credit_union', saveit=True, fp='app/data/aws/'):
    domains = Domain.objects.filter(company__company_type=entity_type, company__is_active=True)
    if id:
        domains = [Domain.objects.filter(company__number=id, company__company_type=entity_type, company__is_active=True).first()]
    for d in domains:
        r = call_api(d.name, call_type)
        if r:
            fn = ('%s.json')%(d.name)
            save_json(r, fn, fp + call_type.lower())
        else:
            print('error with ', d.name)
        time.sleep(SLEEP)


#https://awis.alexa.com/developer-guide
def call_api(url='yahoo.com', call_type='UrlInfo|TrafficHistory|SitesLinkingIn'):
    rurl = 'https://awis.api.alexa.com/api?'
    action = call_type
    output = 'json'
    headers = {'x-api-key': settings.AWIS_KEY}
    if call_type == 'UrlInfo':
        response_group = 'Rank,SiteData,UsageStats,Speed,LinksInCount'
    elif call_type == 'TrafficHistory':
        response_group = 'History'
    elif call_type == 'SitesLinkingIn':
        response_group = 'SitesLinkingIn'

    u = rurl+'Action='+action+'&ResponseGroup='+response_group+'&Url='+url+'&Output='+output
    r = requests.get(u, headers=headers)
    print(r.text)

    j = json.loads(r.text)
    root_json = j['Awis']['Results']['Result']['Alexa']
    for k in root_json.keys():
        print(root_json[k])
    if 'TrafficData' in root_json:
        print(root_json['TrafficData'])
    if 'ContentData' in root_json:
        print(root_json['ContentData'])
    if 'TrafficHistory' in root_json:
        print(root_json['TrafficHistory'])
    return root_json
